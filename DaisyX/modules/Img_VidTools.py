...
#    Some content of this file changes by @InukaAsith for DaisyX
...
from DaisyX.function.telethonbasics import convert_to_image, crop_vid, runcmd, tgs_to_gif, progress, humanbytes, time_formatter, is_nsfw, is_admin
...
DEEP_API_KEY = get_str_key("DEEP_API_KEY", None)
sedpath = "./starkgangz/"
...
@borg.on(events.NewMessage(pattern="/cit$"))
async def hmm(event):
    if event.fwd_from:
        return
    ...
    if await is_admin(event, event.message.sender_id):      
      ...
      return              
@borg.on(events.NewMessage(pattern="/getthumb$"))
...
#    if await is_admin(event, event.message.sender_id):   
    if not event.reply_to_msg_id:
        await event.reply("`Reply to any Media File. I will Send You Its Thumb.`")
        return
    is_reply = await event.get_reply_message()
    try:
        thumbstark = await event.client.download_media(is_reply.media, thumb=-1)
    except:
        await event.reply("`Well, My Eyes Couldn't Find Any Thumb. :/`")
        return
    await event.delete()
...
@borg.on(events.NewMessage(pattern="/nst$"))
async def hmm(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):       
...
@borg.on(events.NewMessage(pattern="/nsfw$"))
async def _(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID): 
    ...
    #if await is_admin(event, event.message.sender_id):   
    if not event.reply_to_msg_id:
        await event.reply("Reply To Any Image Idiot.")
        return
    reply_message = await event.get_reply_message()
    kok = await event.reply("`Processing...`")
...
@borg.on(events.NewMessage(pattern="/color$"))
async def _(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
...
@borg.on(events.NewMessage(pattern="/picgen$"))
async def _(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
    ...
    if await is_admin(event, event.message.sender_id):      
      ...
      poppy = await event.reply("Creating a fake face for you... 🌚")
      if response.status_code == 200:
        with open("FRIDAYOT.jpg", 'wb') as f:
          f.write(response.content)

      captin = f"Fake Image By @DaisyXBot.\nSay Hi to the support @DaisySupport_Official.\nInspired by @FridayOT"
      fole = "FRIDAYOT.jpg"
...
@borg.on(events.NewMessage(pattern="/thug$"))
...
@borg.on(events.NewMessage(pattern="/msk$"))
...
@borg.on(events.NewMessage(pattern="/toonify$"))
async def toony(event):
    ...
    if await is_admin(event, event.message.sender_id): 
        if not event.reply_to_msg_id:
            await event.reply("Reply to any Image.")
            return
        hmmu = await event.reply("`Converting Toonized Image..`")
        await event.get_reply_message()
        img = await convert_to_image(event, borg)
...
@borg.on(events.NewMessage(pattern="/tig$"))
...
@borg.on(events.NewMessage(pattern="/geyuser$"))
async def lolmetrg(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
        ...
        return
    if await is_admin(event, event.message.sender_id): 
        await event.reply("`Meking This Guy Gey.`")
        sed = await event.get_reply_message()
        img = await convert_to_image(event, borg)
        url_s = upload_file(img)
        imglink = f"https://telegra.ph{url_s[0]}"
...
@borg.on(events.NewMessage(pattern="/pix$"))
async def lolmetrg(event):
    if not await is_admin(event, BOT_ID):        
        await event.reply("`I Should Be Admin To Do This!`")
        return
    if await is_admin(event, event.message.sender_id): 
        await event.reply("`Pixing This Image.`")
        sed = await event.get_reply_message()
        img = await convert_to_image(event, borg)
        url_s = upload_file(img)
        imglink = f"https://telegra.ph{url_s[0]}"
...
@borg.on(events.NewMessage(pattern="/ytc$"))
async def lolmetrg(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
        ...
        return
    if await is_admin(event, event.message.sender_id): 
        await event.reply("`Making Comment`")
        sed = await event.get_reply_message()
...
@borg.on(events.NewMessage(pattern="/jail$"))
async def hmm(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
...
@borg.on(events.NewMessage(pattern="/greyscale$"))
async def hmm(event):
    if event.fwd_from:
        return
    if event.fwd_from:
    ...
    if await is_admin(event, event.message.sender_id): 
        if not event.reply_to_msg_id:
            await event.reply("Reply to any Image.")
            return
        hmmu = await event.reply("hmm.. Creating a black&White image...")
        await event.get_reply_message()
        img = await convert_to_image(event, borg)
        img1 = cv2.imread(img)
...
# TG ~>>//@CharonCB21
# @code-rgb
@borg.on(events.NewMessage(pattern="/fgs (.*)"))
async def img(event):
    if event.fwd_from:
        return
    if event.fwd_from:
    ...
    if await is_admin(event, event.message.sender_id): 
        ...
        if not text:
            await event.reply("No input found!")
            return
        if ";" in text:
...
@borg.on(events.NewMessage(pattern="/lg$"))
async def lottiepie(event):
    ...
    if not await is_admin(event, BOT_ID):        
        ...
        return
    if await is_admin(event, event.message.sender_id): 
        await event.reply("`Prooooooccccesssssssinggggg.....`")
        ...
        if message.media and message.media.document:
            ...
            await event.delete()
            await borg.run_cmd(f"lottie_convert.py json.json tgs.tgs")
...
@borg.on(events.NewMessage(pattern="/phcomment ?(.*)"))
async def img(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
...
@borg.on(events.NewMessage(pattern="/spin ?(.*)"))
async def spinshit(message):
    ...
    if await is_admin(event, event.message.sender_id): 
        ...
        if not lolshit.isdigit():
            await message.reply("`Only Speed from 1-6 Is Allowded !`")
            return
        if int(lolshit) > 6:
            await message.reply("`Only Speed from 1-6 Is Allowded !`")
            return
        ...
        if not pic_loc:
            ...
            return
        await message.reply("🌀 `Tighten your seatbelts, sshhT*t is about to get wild ...`")
        spin_dir = 1
...
@borg.on(events.NewMessage(pattern="/lnews ?(.*)"))
async def hmm(event):
    if event.fwd_from:
        return
    if not await is_admin(event, BOT_ID):        
...
@borg.on(events.NewMessage(pattern="/gencertificate ?(.*)"))
async def holastark2(event):
    ...
    if not await is_admin(event, BOT_ID):        
        ...
        return
    if await is_admin(event, event.message.sender_id):
        famous_people = ['Modi', 'Trump', 'Albert', 'Tony-stark', 'Stark-Gang', 'Gandhi','InukaAsith','TeamDaisyX']
...
@borg.on(events.NewMessage(pattern="/slogo ?(.*)"))
...
@borg.on(events.NewMessage(pattern="/adityalogo ?(.*)"))
async def yufytf(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        ok = sedpath + "/" + file_name
        img.save(ok, "PNG")
        await borg.send_file(event.chat_id, ok, caption="Made By @DaisyXBot using the image_videtools plugin written by @FridayOT")
        if os.path.exists(ok):
            os.remove(ok)
    else:      
      await event.reply("`You Should Be Admin to do this.. Always free in my inbox!`")
...
@borg.on(events.NewMessage(pattern="/certificategen ?(.*)"))
async def holastark(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        cv2.imwrite(ok, img)
        await event.delete()
        await borg.send_file(event.chat_id, file=ok, caption="Powered By @FridayOT, Made by @DaisyXBot")
        if os.path.exists(ok):
            os.remove(ok)
    else:      
      await event.reply("`You Should Be Admin to do this.. Always free in my inbox!`")
      return       
@borg.on(events.NewMessage(pattern="/modify (flip|blur|tresh|hsv|lab|sketch)"))  
async def warnerstark_s(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        if ws == "flip":
            ...
            warnerstark = "Hehe, Drawn By @DaisyXBot.. Powered by @FridayOT"
        await event.delete()
...
@borg.on(events.NewMessage(pattern="/mediatoimg$"))
async def warnerstarkgang(event):
    ...
    if await is_admin(event, event.message.sender_id):
        img = await convert_to_image(event, borg)
        await event.reply("`Coverting This Media To Image Now.`")
        so = "**Powered By @DaisySupport_Official.. Inspired by @FRIDAYOT**"
        await event.delete()
...
async def asscompress(event):
    ...
    if await is_admin(event, event.message.sender_id):
        if not event.reply_to_msg_id:
            await event.reply("Reply To Image To Compress It")
            return
        sed = await event.get_reply_message()
        if not sed.photo:
            await event.reply("This Only Works On Images")
            return
        ...
        ok = "Join@DaisySupport_Official.png"
        im.save(ok, optimize=True, quality=sedq)
        await event.reply("`Compressing This Image.`")
        so = "**Powered By @DaisySupport_Official.. Inspired by @FRIDAYOT**"
        await event.delete()
...
async def _(event):
    ...
    if not await is_admin(event, BOT_ID):        
        ...
        return
    if await is_admin(event, event.message.sender_id):
        await event.reply("Oh Wait Let Me Get Wear Glasses")
        if not event.reply_to_msg_id:
...
async def fasty(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        await borg.send_file(
            event.chat_id,
            final_file,
            caption="**Fast Motion** - Powered By @DaisySupport_Official.. Inspired by @FRIDAYOT")
...
async def fasty(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        await borg.send_file(
            event.chat_id,
            final_file,
            caption="**Slow Motion** - Powered By @FDaisySupport_Official")
        await event.delete()
        for files in (filem, hmm):
            if files and os.path.exists(files):
                os.remove(files)
...
async def flip(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        await friday.run_cmd(cmd)
        filem = "FlipedByDaisywith@FridayOT.mp4"
        if not os.path.exists(filem):
        ...
        await borg.send_file(
            event.chat_id,
            final_file,
            caption="**Video Flipped** - Powered By @DaisySupport_Official.. Inspired by @FRIDAYOT")
        for files in (filem, hmm):
            if files and os.path.exists(files):
                os.remove(files)
    else:      
...
async def audio_extract(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        kk = await event.get_reply_message()
        if not kk.video or kk.video_note:
            await event.reply("`You fool!, Reply to a video`")
            return
        hmm = await event.client.download_media(kk.media)
        try:
        ...
        await borg.send_file(
            ...
            final_file,
            thumb=thumb,
            caption="**Audio Extarcted** - Powered By @DisySupport_Official.. Inspired by @FRIDAYOT")
        for files in (filem, hmm):
            if files and os.path.exists(files):
                os.remove(files)
    else:      
...
async def starkmeme(event):
    ...
    if await is_admin(event, event.message.sender_id):
        ...
        if hmm == None:
            await event.reply("Give Some Text to Memify")
            return
        if not event.reply_to_msg_id:
            await event.reply("`PLease, Reply To A MsG`")
            return
        mryeast = await event.reply("Vro, Making a meme.. Wait")
        await event.get_reply_message()
...
async def glitch(event):
    ...
    if await is_admin(event, event.message.sender_id):
        if not event.reply_to_msg_id:
            await event.reply("`Reply to a Image vro`")
            return
        sed = await event.get_reply_message()
        okbruh = await event.reply("`Wtf.. Gli, Glitchiiingggg.....`")
        ...
        optimize(pathsn)
        await event.edit("Starting Upload.. Join @DaisySupport_Official")
        stark_m = await uf(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, event, c_time, "Uploading..Join @DaisySupport_Official", pathsn
                    )
                ),
            )
        await borg.send_file(event.chat_id,
                             stark_m,
                             caption="Powered By @DaisySupport_Official.. Inspired by @FRIDAYOT")
        await okbruh.delete()
